...
func render(w http.ResponseWriter, r *http.Request, status int, file string, data interface{}) {
	fmap := template.FuncMap{
		...
		"getEntry": func(id int) Entry {
			row := db.QueryRow(`SELECT id,user_id,private,title,first_row,body,created_at FROM entries WHERE id=?`, id)
			var entryID, userID, private int
			var body, title, firstRow string
			var createdAt time.Time
			checkErr(row.Scan(&entryID, &userID, &private, &title, &firstRow, &body, &createdAt))
			return Entry{id, userID, private == 1, title, firstRow, createdAt}
		},
...
func GetIndex(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query(`SELECT id,user_id,private,title,first_row,body,created_at FROM entries WHERE user_id = ? ORDER BY created_at LIMIT 5`, user.ID)
	if err != sql.ErrNoRows {
		checkErr(err)
	}
	...
	for rows.Next() {
		...
		entries = append(entries, Entry{id, userID, private == 1, title, firstRow, createdAt})
	}
	rows.Close()
...
DESC LIMIT 500`, friendIds)
	...
	for _, relation := range relationsAnother {
		if _, ok := friendsMap[relation.One]; !ok {
			friendsMap[relation.One] = relation.CreatedAt
		}
	}
	for _, relation := range relationsOne {
		if _, ok := friendsMap[relation.Another]; !ok {
			friendsMap[relation.Another] = relation.CreatedAt
		}
	}
...
func GetEntry(w http.ResponseWriter, r *http.Request) {
	...
	row := db.QueryRow(`SELECT id,user_id,private,title,first_row,body,created_at FROM entries WHERE id = ?`, entryID)
	var id, userID, private int
	var body, title, firstRow string
	var createdAt time.Time
	err := row.Scan(&id, &userID, &private, &title, &firstRow, &body, &createdAt)
	if err == sql.ErrNoRows {
		checkErr(ErrContentNotFound)
	}
	checkErr(err)
